...
<head>
    ...
    <title>Detalle Cierre Caja #{{ $session->caja_id }}</title>
...
<body>
    <div class="center bold">DETALLE CIERRE DE CAJA #{{ $session->caja_id }}</div>
    ...
    <table class="tbl">
        ...
        @forelse($session->movements as $movement)
            ...
            <tr>
                <td class="small">{{ $movement->created_at?->format('H:i') }}</td>
                <td class="small bold">{{ $typeLabel }}</td>
                <td class="small right">${{ number_format((float)$movement->amount, 2) }}</td>
            </tr>
            <tr>
                <td colspan="3" class="small muted">{{ $movement->reason }}</td>
            </tr>
        @empty
