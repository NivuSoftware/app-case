<x-app-layout>
    ...
    function cargarProveedores() {
        fetch(`${PROVEEDORES_BASE}/list`)
            ...
            .catch(err => {
                console.error(err);
                if (window.Swal) {
                    Swal.fire('Error', 'No se pudo cargar el listado de proveedores.', 'error');
                } else {
    ...
    function closeCreateModal() {
        ...
        ['c-nombre','c-ruc','c-telefono','c-email','c-direccion','c-contacto']
            .forEach(id => {
    ...
    function openEditModal(id) {
        const p = TODOS_PROVEEDORES.find(s => s.id === id);
        if (!p) return;
    ...
    async function updateProveedor() {
        ...
        const contacto  = document.getElementById('e-contacto').value.trim();
